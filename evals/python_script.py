"""Python script eval adapter — runs a Python script and parses the metric from stdout."""
...
class PythonScriptAdapter(EvalAdapter):
    """Run a Python eval script.

    Config:
        script_path: Path to the Python eval script (relative to workspace)
        timeout_seconds: Override time budget (optional)
    """

    def run(self, workspace: Path, metric_name: str) -> EvalResult:
        script_path = self.config.get("script_path", "eval.py")
